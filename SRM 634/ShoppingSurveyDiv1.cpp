...
// CUT begin
ifstream data("ShoppingSurveyDiv1.sample");
...
bool do_test(int N, int K, vector<int> s, int __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(N, K, s, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1502846910;
