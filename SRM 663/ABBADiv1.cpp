...
void dfs(string cur) {
    if (cur.size() == 0 || S.find(cur) != S.end()) {
        return;
    }
    S.insert(cur);
    if (cur.back() == 'A') {
        string tmp(cur.begin(), cur.end() - 1);
        dfs(tmp);
    }
    if (cur[0] == 'B') {
        string tmp(cur.begin() + 1, cur.end());
...
class ABBADiv1 {
    ...
    string canObtain(string initial, string target) {
        dfs(target);
        return S.find(initial) == S.end() ? "Impossible" : "Possible";
    }
};
