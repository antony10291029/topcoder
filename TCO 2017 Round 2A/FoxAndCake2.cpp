...
class FoxAndCake2 {
    public:
    bool isOk(int c, int s) {
        if (c > s) swap(c, s);
        if (gcd(s, c) > 1) return true;
        if (c == 2) return gcd(c, s) > 1;
        if (c == 3) return gcd(c, s) > 1;
        if (c == 4) return gcd(c, s) > 1;
        if (c == 5) return s >= 7;
        if (c == 6) return gcd(c, s) > 1;
        if (c >= 7) return true;
        return false;
    }
        
    string isPossible(int c, int s) {
        return isOk(c, s) ? "Possible" : "Impossible";
    }
};
