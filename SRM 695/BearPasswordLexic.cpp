...
class BearPasswordLexic {
    ...
    string findPassword(vector<int> x) {
        ...
        if (tot != n) return string("");
        string res = "";
        int low = 0, high = all.size() - 1;
        int rev = 0;
        while (low <= high) {
            if (!rev) {
                for (int i = 0; i < all[high]; ++i) res += 'a';
                rev = 1 - rev;
                --high;
            } else {
                for (int i = 0; i < all[low]; ++i) res += 'b';
                rev = 1 - rev;
                ++low;
            }
        }
...
// CUT begin
ifstream data("BearPasswordLexic.sample");
